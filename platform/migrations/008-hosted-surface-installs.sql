CREATE TABLE surface_installs_hosted (
 tenant_id TEXT NOT NULL, project_id TEXT NOT NULL, id TEXT NOT NULL,
 framework TEXT NOT NULL CHECK(framework IN('nextjs-app','react-router','dom','vite-react-fastapi','hosted-script')),
...
INSERT INTO surface_installs_hosted
SELECT tenant_id,project_id,id,framework,mode,application_origin,route_path,nav_label,
 bridge_path,slot_id,environment,credential_hash,bundle_hash,status,route_mounted,
 bridge_reachable,authority_configured,last_seen_at,verified_at,last_error,created_by,
 created_at,revoked_at,design_fingerprint
FROM surface_installs;

DROP TABLE surface_installs;
ALTER TABLE surface_installs_hosted RENAME TO surface_installs;
