...
Surface installation records bind a generated integration bundle to one project, declared slot, exact customer origin, target profile, route, same-origin bridge and human-approved design-contract fingerprint. The framework-agnostic core models client runtime, client language, build tool and server framework separately, then delegates file generation to an exact target adapter. Vite React + FastAPI is native on both sides and does not add a Node proxy. A public verification credential, returned only in the generated bundle, reports operational mount facts and is not host authority. The host token, confirmation key, session mapping and business authorization remain exclusively server-side. Re-publication can improve an existing Atelier-owned slot without changing customer routing; arbitrary host DOM or navigation remains outside Atelier's mutation boundary.
