...
**Runtime plane:** one Atelier-hosted module script → origin-bound public manifest → signed current release, approved design contract and reviewed agent profile → customer-owned mount → hosted surface/chat UI → reviewed same-origin HTTP operation allowlist → existing customer browser session → customer API authentication, tenant/object permission and business rules. Atelier issues a sealed short-lived conversation session for transcript ownership and model turns; it is not customer API authority. The browser never receives an Atelier project token or customer API credential. A published screen does not call a model on ordinary render or interaction; the chatbot calls the configured model only for explicit turns.
...
**Trust domains:** the control plane stores and approves capabilities/artifacts and serves UI; the customer API owns business data and authority; the customer browser carries its existing application session; project-bound CLI workers own isolated model accounts; the source certifier executes without network or control-plane credentials; coding agents receive only an explicitly scoped read token. The public embed key selects an origin-bound UI manifest and is neither a bearer credential nor API authority. No compiler prompt can acquire host permissions.
...
Surface installation records bind one hosted-script embed to a project, declared slot, exact customer origin, page, environment and human-approved design-contract fingerprint. Studio returns one customer-owned mount snippet and a public origin-bound install key. The key can retrieve only the current signed UI manifest, configured hosted-chat metadata and exact reviewed HTTP operation metadata; it cannot call Atelier administration APIs or grant customer API authority. Hosted chat additionally requires a reviewed current profile, a real project provider, and browser execution for every tool. The browser client rejects unknown operations and cross-origin URLs; commands display an explicit confirmation before the registered call. Re-publication improves the same Atelier-owned slot without changing customer routing; arbitrary host DOM or navigation remains outside Atelier's mutation boundary.
...
After deterministic approval, an optional agentic Design Genome stage may interpret the sanitized contract into density, hierarchy, interaction-tone and reusable-pattern guidance. Every claimed pattern must cite exact role/property/value evidence from the approved contract, is rejected if that evidence does not match, and remains a draft until a human approves it. Surface generation receives approved semantic guidance alongside the unchanged deterministic contract; the synthesis cannot add or alter tokens, capabilities, selectors or authority.
