...
**Runtime plane:** one Atelier-hosted module script → origin-bound public manifest → signed current release and approved design contract → customer-owned mount → reviewed same-origin HTTP operation allowlist → existing customer browser session → customer API authentication, tenant/object permission and business rules. The browser never receives an Atelier project token or customer API credential. A published screen does not call a model on ordinary render or interaction.
...
Surface installation records bind one hosted-script embed to a project, declared slot, exact customer origin, page, environment and human-approved design-contract fingerprint. Studio returns one customer-owned mount snippet and a public origin-bound install key. The key can retrieve only the current signed UI manifest and the exact reviewed HTTP operation metadata used by that release; it cannot call Atelier administration APIs or grant customer API authority. The hosted browser client rejects unknown operations and cross-origin URLs. Re-publication improves the same Atelier-owned slot without changing customer routing; arbitrary host DOM or navigation remains outside Atelier's mutation boundary.
