...
| Design observation leakage    | Explicit marked root, fixed computed-style allowlist, no text/HTML/form values, server validation and human approval                  |
| Specialist privilege widening | Read-only subset tools, one-level delegation, bounded rounds, encrypted internal notes and primary synthesis                          |
| Install receipt forgery       | Origin- and bundle-bound public key, rate limit, factual fields only, revocation, receipt never grants host or capability authority   |

## Explicit residual risks

Static source policy and a subprocess are not a complete hostile-code sandbox. Production source certification therefore refuses local mode, but the container image and runtime configuration still require deployment-specific build, scan, and penetration evidence. SQLite offers durable single-host operation, not multi-host HA. Administrators controlling the process, database, and keys are inside the trust boundary. Heuristic PII/secret detection is not DLP certification. Browser observer and install-verification keys are visible, and Origin can be forged outside a browser, so browser observations and install receipts are operational evidence only; they never grant capability, agent or host authority.
