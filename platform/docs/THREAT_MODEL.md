...
Static source policy and a subprocess are not a complete hostile-code sandbox. Production source certification therefore refuses local mode, but the container image and runtime configuration still require deployment-specific build, scan, and penetration evidence. SQLite offers durable single-host operation, not multi-host HA. Administrators controlling the process, database, and keys are inside the trust boundary. Heuristic PII/secret detection is not DLP certification.
