...
For entity-specific pages, the host may expose a small synchronous context hook before the module loads, for example `window.AtelierHost.context = () => ({ findingId: currentFindingId })`. Return only the identifiers and coarse page state the reviewed tools need; never include credentials, raw private records or unconstrained page content. Atelier adds the current route itself and enforces the profile's 8 KB context limit.

Surface generation includes a query with required inputs only when the selected slot's `contextSchema.required` guarantees every non-header input. Queries without that guarantee stay available to the chatbot for explicit multi-step tool use, but are not executed eagerly by a rendered surface. Required headers are never accepted as browser context because the hosted client deliberately ignores credential headers.
