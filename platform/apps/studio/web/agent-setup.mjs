...
export function agentSetupFields({ profile, project, enabled, hasProvider, e, pill }) {
  const selected = new Set(
    ...
    firstSetup = !profile;
  return `<label>Assistant name<input name="name" required value="${e(profile?.name ?? `${project.name} assistant`)}"></label><label>Product voice<textarea name="tone" required>${e(profile?.voice?.tone ?? 'Clear, calm and precise')}</textarea></label><div class="two-fields"><label>Locale<input name="locale" value="${e(profile?.voice?.locale ?? 'en')}" required></label><label>Retention days<input name="retentionDays" type="number" min="1" max="90" value="${profile?.retentionDays ?? 30}" required></label></div><fieldset class="tool-select"><legend>Approved chatbot tools</legend>${enabled.map((capability) => `<label class="checkbox"><input type="checkbox" name="tools" value="${e(capability.id)}" ${selected.has(capability.id) ? 'checked' : ''}>${e(capability.title ?? capability.id)} ${pill(capability.kind)}</label>`).join('')}</fieldset><fieldset class="tool-select specialist-select"><legend>Deep-answer specialists</legend><p class="help">Each enabled specialist is a separate, auditable model turn. It receives only the selected read tools and can never run commands or delegate again.</p>${presets
    .map((preset) => {
