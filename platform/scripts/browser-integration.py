    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            def onboarding_view():
                ...
                install.locator('select[name="target"]').select_option('vite-react-fastapi')
                ...
                expect(ready.get_by_role('heading',name='Vite React + FastAPI',exact=True)).to_be_visible()
                expect(ready.get_by_text('Fail-closed host authority and capability adapters',exact=True)).to_be_visible()
                expect(ready.get_by_text('Same-origin FastAPI bridge router',exact=True)).to_be_visible()
