...
export class AtelierDomSlot {
  constructor({ element, runtime, context, fallback = '', onActionError = null } = {}) {
  ...
  async mount() {
    const slot = this.runtime.slots.get(this.context.slotId);
    const resolved = await this.runtime.resolver.resolve(this.context, slot);
    if (!resolved.bundle) {
      this.element.innerHTML = this.fallback;
      return resolved;
    }
    const dataByCapability = {};
    await Promise.all(
      resolved.bundle.experiencePlan.queryPlan.map(async (query) => {
        try {
          dataByCapability[query.capabilityId] = await this.runtime.data.resolve(
            { source: query.capabilityId, select: query.fields },
            this.context,
          );
        } catch (error) {
          dataByCapability[query.capabilityId] = error;
        }
      }),
    );
    this.element.innerHTML = renderBundle(resolved.bundle, dataByCapability);
    this.element.addEventListener(
      'click',
      async (event) => {
        const button = event.target.closest?.('[data-atelier-action]');
        if (!button) return;
        try {
          await this.runtime.dispatcher.dispatch(
            button.dataset.atelierAction,
            this.context.actionInput ?? {},
            { ...this.context, bundleId: resolved.bundle.bundleId },
          );
        } catch (error) {
