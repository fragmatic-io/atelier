...
- [x] **Real permission grant flow.** Landed alongside V-1. The demo's onboarding "Grant" call now mints a scoped token via `VaultClient.requestGrant()`; revoking from `/settings/intent` calls `client.revokeGrant()` which cascades a `system.security_revocation` trigger so subscribed runtimes invalidate manifests compiled from the affected slice.
- [x] **V-3 — Permission grant UI against the real vault.** Landed in Wave 8. The demo no longer mints tokens itself — `requestGrant()` redirects the browser to `${NEXT_PUBLIC_VAULT_URL}/vault/consent?...`, the vault renders an OAuth-style consent screen (server-rendered HTML, no JS framework), the user approves, the vault mints a token and redirects back to `/onboarding/grant-callback?token=<jwt>`. CSRF is bound by an HMAC nonce paired between a `cir_vault_consent_nonce` cookie and a hidden form field. Per-jti revocation hits the vault's `DELETE /vault/grants/:jti` endpoint and cascades `system.security_revocation`. New files: `packages/vault-server/src/consent.ts` + tests, `apps/demo/lib/intent-grant.ts` + tests, `apps/demo/app/onboarding/grant-callback/page.tsx`. Follow-up: rich consent-screen styling (current pass is utilitarian inline CSS), on-vault profile seeding endpoint (today the demo bootstraps via PATCH which 404s on a fresh user — the callback falls back to localStorage with a hint).
...
- [x] **V-3** — Permission grant UI against the real vault. Landed Wave 8 (see "Vault + permissioning" entry above for details).
...
- [x] **DD** — Full personalisation chain integration eval. Shipped at `evals/end-to-end/personalisation-chain.eval.ts` (+ sibling `.test.ts`). Walks intent → `CompositeCompiler[Fallback, Gemini?]` → manifest validate → `BASELINE_POLICIES` → render-plan walker → simulated engagement (`StreamingAuditSink` + `BehavioralTap` + `SequenceDetector`) → refined compile that emphasizes the frequently-clicked capability. Deterministic offline; Gemini head wired but never load-bearing for the assertions. Demo's `cir-server.ts` now seeds a live `SequenceDetector` from real audit events via `BehavioralTap` so the same loop runs in production.
...
- [x] **Vis-2 — Dark mode as a real surface across all 62 components.** Every entry in `_variants.ts` ships paired light + `dark:` Tailwind utilities (action / layout / display / stat / search / codeBlock / tooltip / hoverCard / statusBar / bulkActionBar / pinnedSeparator). Hardcoded inline classes in `BulkActionBar` got `dark:` siblings too. `cir init` now scaffolds `tailwind.config.mjs` with `darkMode: ['class', '[data-color-mode="dark"]']` so the runtime's `<html data-color-mode>` mirror matches automatically. Regression gate at `test/_variants-dark.test.ts` asserts every colour-bearing variant carries a `dark:` prefix. **DONE.**
