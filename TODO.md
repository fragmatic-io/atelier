...
- [ ] **Int-8** — Undo toast on every destructive action. Linear ships an undo toast with a 5s window. New `withUndo()` middleware on `ActionDispatcher` + capability `undoable: true` flag + `<Toast variant="undo">`. **1 wk.** (We already have the policy + ambient satisfier; this wires the runtime middleware.)
...
- [ ] **Cnt-1** — Code rendering at depth. Syntax highlighting (Shiki or Starry-Night), fold, line references, inline comments. **2 wk.**
