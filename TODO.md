...
- [ ] **Visual baseline PNG regeneration.** Schema/persona fix above (`f36e698`) unblocks the LLM-compile path; the placeholder solid-colour PNGs in `packages/components/storybook/__compiled-workflow-baselines/` still need to be regenerated against real Gemini output via `GEMINI_API_KEY=<key> pnpm components:storybook:visual:compiled:update-baselines` and committed. Operator-with-key task; agent envs lack the key. **5-10 min.**
