...
- [ ] **Hosted persona corpus for `marketplace-eval-llm.yml`.** First workflow run today (2026-05-05, post-fix `967c5b5`) exits clean but reports 0 personas because `MARKETPLACE_VAULT_URL` defaults to a placeholder. Need either (a) a hosted vault endpoint with seeded approved personas, or (b) a `--local-fixtures` flag that points at the in-tree `recipes/` directory + the `LocalRecipeStore` impl. Until then the gate verifies wiring but produces no real cost data. **2-3 d** (option b is simpler).
