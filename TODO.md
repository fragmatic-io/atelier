...
- [ ] **Vis-10** — Notification badge system. Slack/Discord/Linear all ship grouped per-domain badges. Builds on `<MetaBadge>` from Wave M. **1 wk.** Depends on Vis-9. **Natural follow-on now that Vis-9 has shipped** — the click-through + capability-binding pattern Vis-9 introduces (`data: { source: '…' }` + `href` / `onClick`) is the same shape the badge primitive will adopt for `notifications.unread_count` / per-domain grouping; reuse `data-cir-bound` + the lucide severity-icon slot for category icons.
