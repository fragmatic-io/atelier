...
| **11**  | Visual depth — Vis / Int / Cnt / Nav / Coll / AI (~50 items; Vis-2, Int-2, Int-3, Int-4, Int-11, Int-13, Cnt-1, Cnt-3, Cnt-4, Cnt-5, Cnt-8, Nav-4 ✅ shipped)  | 📅 partial     | varies              | 6-10 mo    | C, P-7, S-3    |
...
- [x] **Int-15** — Smart paste with link unfurl. **Shipped** — `useSmartPaste({ embedRegistry, onPaste, unfurlTimeoutMs? })` hook in `@atelier/react` (`packages/react/src/hooks/use-smart-paste.ts`) detects the first URL in pasted clipboard text, dispatches it to the host's Cnt-4 `EmbedRegistry`, and races resolution against the configured timeout (default 1500 ms): on success fires `onPaste({ text, unfurled: { url, display } })`, on miss / null / rejection / timeout falls back to `onPaste({ text })`. URL detection trims trailing prose punctuation while preserving balanced parens (Wikipedia-style `foo_(bar)`), `http://` / `https://` schemes only, FIRST URL only (Notion / Slack semantics — manual unfurl on subsequent links). `EmbedRegistry` / `EmbedDisplay` types are structurally mirrored in the hook so `@atelier/react` stays free of a runtime dep on `@atelier/components` (`useTrail` / `useUndoToastEmitter` precedent). `<RichText pasteSmart={{ embedRegistry, onUnfurl }}>` opt-in in `@atelier/components` exposes the same contract on the editor surface (talks to the registry directly to avoid the components → react dep cycle). Default paste behaviour is NEVER cancelled — the editor inserts the raw text and the unfurl callback fires alongside so the host composes its own UI on top. 19 hook tests (URL detect / resolve / timeout fallback / null fallback / reject fallback / no URL / no clipboard / opt-out via `unfurlTimeoutMs: 0` / handler stability + ref tracking / DOM bind smoke) + 8 RichText integration tests + 3 detect-URL helper tests. Catalog unchanged (Int-15 is a hook, not a component). Notion / Slack reference.
...
- [ ] **Cnt-9** — Activity feed with diff visualization. Linear's activity feed renders typed events with collapse-by-default diffs. New `<ActivityFeed>` primitive consuming an `activity.list` capability shape. **2 wk.** Cnt-2 ✅ unblocked.
...
- [ ] **AI-1** — "Ask AI" on selection. Notion's "Ask AI" + "Summarize" + "Improve writing" floating bar over selected text. New `<SelectionActionBar>` + capability-typed prompt registry per surface. **2 wk.** Cnt-7 ✅ unblocked.
