...
- [x] **Hosted persona corpus for `marketplace-eval-llm.yml`** (option b shipped). `runMarketplaceEval({ localFixtures: { directory } })` walks a directory of `*.json` / `*.recipe.json` recipe files, synthesises a canonical `atelier://local/<file>@1.0.0` address per persona, runs the same compile + validation pipeline as the vault-fetch path, skips signature verification (files are unsigned) and records `signature_verified: false` per persona. CLI exposes the seam as `--local-fixtures <dir>` (mutually exclusive with `--vault-url`). The `marketplace-eval-llm.yml` workflow now defaults to local-fixtures against the in-tree `recipes/` directory; setting `MARKETPLACE_VAULT_URL` flips it back to the hosted path when one ships. The 7-day clean-clock starts producing meaningful signal. _This commit._
- [ ] **Bootstrap visual baselines from real Gemini compile.** Workflow personas (`approval-command-center` / `customer-context-panel` / `exception-review-workbench`) fail `ManifestSchema` validation because the LLM emits structured filter objects (`{ field, op, value }`) where `ComponentDataBindingSchema.filter` requires a string. Either (a) tighten the persona prompts to insist on string filters, (b) widen the schema to accept object filters with a serialiser hook, or (c) accept whatever Gemini produces today (the local debug shows 1/3 personas does produce a graceful error-state Alert) as the baseline. Visual gate is wired and will fire on first regeneration; the baselines themselves are still placeholder solid-colour PNGs. **1 d** of persona engineering.
