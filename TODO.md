...
- [x] **Sprint 2.3 — visual regression diffs for the 3 workflow composites.** Compile each operational composite (`ApprovalCommandCenter` / `CustomerContextPanel` / `ExceptionReviewWorkbench`) via real Gemini against a pinned persona fixture (`packages/components/storybook/workflows/persona-fixtures/*.persona.json`), render the resulting `Manifest` to a static HTML document via the new `renderManifestToHtml()` helper (`packages/components/storybook/workflows/render-manifest-to-html.tsx`), screenshot at 1280×720, and diff against `__compiled-workflow-baselines/<workflow>-1280x720.baseline.png` at a 5% pixel threshold. Spec: `packages/components/storybook/compiled-workflows.playwright.ts`; config: `packages/components/storybook/playwright.compiled-workflows.config.ts`; CI: new `.github/workflows/compiled-workflows-visual.yml` (gated on `secrets.GEMINI_API_KEY`, daily schedule + push to `main` + non-fork PRs). New scripts: `pnpm components:storybook:visual:compiled` + `pnpm components:storybook:visual:update-baselines`. Operator docs: `apps/docs/src/content/docs/operations/visual-regression.mdx`. _This commit._
- [ ] **Rendered-output snapshot diffs (top-10 personas)** — Sprint 2.3 covers the 3 operational composites; the larger top-10 marketplace persona surface still needs a parallel gate. Reuses the same `compileWorkflowViaLlm` + `renderManifestToHtml` plumbing. **1 wk.**
- [x] **Hosted persona corpus for `marketplace-eval-llm.yml`** (option b shipped). `runMarketplaceEval({ localFixtures: { directory } })` walks a directory of `*.json` / `*.recipe.json` recipe files, synthesises a canonical `atelier://local/<file>@1.0.0` address per persona, runs the same compile + validation pipeline as the vault-fetch path, skips signature verification (files are unsigned) and records `signature_verified: false` per persona. CLI exposes the seam as `--local-fixtures <dir>` (mutually exclusive with `--vault-url`). The `marketplace-eval-llm.yml` workflow now defaults to local-fixtures against the in-tree `recipes/` directory; setting `MARKETPLACE_VAULT_URL` flips it back to the hosted path when one ships. The 7-day clean-clock starts producing meaningful signal. `4d8ea84`.
- [x] **Schema/persona gap closed.** `ComponentDataBindingSchema.filter` widened from `z.string().optional()` to `z.union([z.string(), StructuredFilterSchema]).optional()` — both shapes (the CEL-like string the prompt prefers AND the structured `{ field, op, value, and?, or? }` object the LLM tends to emit) now validate. New `formatFilterAsString()` + `applyFilter()` helpers in `@atelier/runtime/data/filter-utils.ts` give string-only consumers a graceful coerce path. The compiler system prompt + the 3 persona fixtures (`approval-command-center` / `customer-context-panel` / `exception-review-workbench`) updated to nudge toward simple list bindings. _This commit (`f36e698`)._
- [x] **`ComponentDataBindingSchema.sort` widening.** Closed `e890158` — accepts `string | StructuredSort` (`StructuredSortKey[]`); new runtime helpers `isStructuredSort` / `formatSortAsString` / `parseSortString` / `applySort` / `coerceSortToString`; consumer audit across 4 data-resolver adapters + react `useSubscription` + apps/demo.
- [x] **`CompiledFromSchema.capability_version` shape closed `e890158`** — accepts `SemverString | Record<CapabilityId, SemverString>` AND adds optional plural `capability_versions: Record<...>` for the record-shape primary surface. Compiler `2405e74` auto-stamps the entire `compiled_from` block deterministically from `CompileInput` so the LLM never has to fill it in. New optional `CompileInput.componentCatalogVersion` field; defaults to `'0.0.0'`.
- [x] **Info-severity policy violations no longer gate the LLM retry loop.** `2405e74` lifted the demo-github inline filter into `createBaselineManifestValidator` so the storybook compile-via-llm path and any other host get the right semantics for free. Advisories like "DetailView has no empty_state — resolver supplies a default" are info, not regressions, and belong in a separate channel.

The three items above were why the first two real-LLM dispatch runs (25378420054, 25395504667) reported `0 passed, 2 failed`. The eval gate caught both schema mismatches AND the missing-provenance shape — exactly as designed. **Closed.**

- [ ] **Visual baseline PNG regeneration.** Three closures above unblocked `customer-context-panel` (passes standalone via `tsx` debug). The remaining two personas (`approval-command-center`, `exception-review-workbench`) trip on **persona-fixture quality**, not framework rigidity: the personas grant a narrow `intent.granted_fields` set but ask the LLM for layouts that need extra fields. Loosen the granted-fields lists OR refactor the fixtures to ask for fewer projections. **1 d.** _Persona engineering, not framework work._
- [ ] **LLM prompt — confirmation + undo guidance.** Today's third real-LLM dispatch (`25397616230`) compiled cleanly through schema and provenance checks ($0.0470 across 8 routes — `dummyjson-shopper` 5 routes, `github-reviewer` 3 routes) but failed `confirmation_required_for_destructive` (LLM didn't gate destructive actions with `<ConfirmDialog>`) and `reversibility_surfaced` (no undo affordance for reversible actions). Improve the system prompt's hard-rules section to emit explicit per-route checklist for these patterns. **<1 d.** _Prompt engineering — the policies themselves are correct._
