...
- [ ] **Visual baseline PNG regeneration.** Schema/persona fix above (`f36e698`) unblocks `filter` but two more schema-LLM mismatches surfaced when the regen attempt actually ran with a real key (2026-05-05 dispatch + local) — see next two items. Until those land, the LLM compile still trips Manifest validation. **Blocked.**
- [ ] **`ComponentDataBindingSchema.sort` widening.** Captured via real-LLM run of `customer-context-panel` persona: `routes[*].layout.children[*].data.sort` came back as an array of sort criteria, but the schema expects a single string. Same class as the `filter` widening done in `f36e698` — accept `string | array<{ field, direction? }>` and coerce in the runtime data resolver. **<1 d.**
- [ ] **`CompiledFromSchema.capability_version` shape mismatch.** Captured via real-LLM run of `exception-review-workbench` persona: the LLM emits `compiled_from.capability_version` as `Record<CapabilityId, SemverString>` (which is semantically what's used — many capabilities, each with its own version). The schema today says single `SemverString`, which only matches a "catalog snapshot" interpretation. Decide: rename to `capability_catalog_version` (string, snapshot semantics) AND add `capability_versions: Record<...>` for per-capability granularity, OR widen `capability_version` to a union. The first is cleaner for cache keys. **1 d** + migration of existing manifests + `compiled_from` consumer audit.

The two items above are why today's first real-LLM dispatch run (25378420054) reported `2 personas — 0 passed, 2 failed` at $0.0225 USD spent. The eval gate is **working as designed** — it caught both mismatches. The 7-day clean-clock unblocks once both schema-LLM mismatches are closed.
