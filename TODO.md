...
- [x] **V-6.d — Review / curation flow.** `ReviewRecord` + `ReviewState` schemas. Server endpoints: `POST /marketplace/review/<a>/<p>@<v>` (signed by maintainer key — `ReviewerKeyDirectory`), `GET /marketplace/review/<a>/<p>@<v>`, `GET /marketplace/index` (default approved-only, `?include=pending,...` for maintainers). Publish auto-creates a `pending` record on first upload. `@atelier/vault-client` ships `submitReview` / `fetchReview` / `listMarketplace`. `<MarketplaceBrowser>` shows a small state pill when state ≠ approved. CLI: `atelier marketplace review`.
- [x] **V-6.e — Eval gate.** New `@atelier/eval-marketplace` package walks the top-N currently-approved personas (default 10, alphabetical-on-canonical-address ranking — flips to download-count once V-6.d telemetry accrues), consumes via V-6.b, verifies the ed25519 signature, compiles each route through a frozen reference capability + component fixture set (no LLM dep), and re-validates the resulting manifests against `ManifestSchema` + `BASELINE_POLICIES` from `@atelier/policies`. Surfaces an `EvalReport` (per-persona passed/failed/skipped + violations + reference-version SHA-256 hashes). New `.github/workflows/marketplace-eval.yml` runs at 04:00 UTC nightly, uploads the report as a 90-day artifact, and auto-opens (or updates) a `marketplace-eval` issue on regression. CLI shim `pnpm marketplace:eval`; docs `apps/docs/src/content/docs/marketplace/eval-gate.mdx`. Bounded at top 10 to keep runtime under 15 min.
...
**Total V-6**: 4-6 wk. V-6.a / V-6.b / V-6.c / V-6.d / V-6.f shipped; V-6.e (eval gate) outstanding.
