...
7. Publish a reviewed surface, choose its page/placement, and generate the one-script hosted embed. Atelier serves the surface, chatbot and runtime; every reviewed API tool runs directly in the customer's browser through its existing same-origin session.
8. Verify the mount, hosted UI/chat session, browser API allowlist, customer API authorization, tool behavior, command confirmation policy, and browser states before production.
...
| `platform/packages/surface-install`, `embed` | One-script hosted surfaces/chat, reviewed browser API tools, and factual receipts |
