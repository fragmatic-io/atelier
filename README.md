...
| 2   | **Policies**            | `@cir/policies`       | 5 baseline validators (`data_access_within_grant`, `confirmation_required_for_destructive`, `no_pii_in_query_strings`, `rate_limited_actions_show_state`, `reversibility_surfaced`). `BehavioralPatternDetector` interface for Phase-4 runtime to plug into.                                                                                                  | `pnpm --filter @cir/policies test` (43 tests). Or: violate a policy in the demo's manifest and watch the resolver throw `ManifestValidationError` |
...
| 5   | **Components**          | `@cir/components`     | 13 baseline React components (Stack, Card, Container, Grid, Markdown via react-markdown + rehype-sanitize, Table, EmptyState, Button, TextInput, Select, Alert, Spinner, ConfirmDialog). `data-cir-component` selectors for theming. RSC-friendly: only ConfirmDialog needs `'use client'`.                                                                   | `pnpm --filter @cir/components test` (94 tests). Or: import them in any React app                                                                 |
| 6   | **React adapter**       | `@cir/react`          | `<CirRuntime>` provider, `<CirRoute path>` walker, hooks (`useCir`, `useManifest`, `useDispatcher`, `useTrigger`), `ConfirmPortal`, `<CirErrorBoundary>`, `DataResolver` protocol. Every file that uses hooks ships `'use client'` for Next.js App Router compat.                                                                                             | `pnpm --filter @cir/react test` (60 tests under happy-dom)                                                                                        |
| 7   | **Compiler**            | `@cir/compiler`       | LLM-backed manifest producer. **Phase 5a in progress** — Gemini integration, prompt builder, response validation, server-side `ManifestStore` (Tier 3), trigger-driven invalidation, fallback to hand-written manifests when no API key                                                                                                                       | (when Phase 5a lands) check `manifest.compiled` audit events show `compiler_model: 'gemini-2.5-pro'` and `token_cost > 0`                         |
| 8   | **Demo (email triage)** | `apps/demo`           | Next.js 15 App Router app exercising every layer end-to-end. 13 components (10 baseline + DecisionQueue/TaskQueue/ThreadView/UndoBar). Tailwind 4 + `data-cir-component` styling.                                                                                                                                                                             | `pnpm --filter @cir/demo dev` → http://localhost:3000/today                                                                                       |
| 9   | **Demo (DummyJSON)**    | `apps/demo-dummyjson` | **Phase 5b in plan** — same runtime, different domain, real public API, lens-switching showcase                                                                                                                                                                                                                                                               | n/a                                                                                                                                               |
| 10  | **Demo (GitHub)**       | `apps/demo-github`    | **Phase 5c-d in plan** — real auth, real mutations, `verbal_required` confirmation, rate-limit policy                                                                                                                                                                                                                                                         | n/a                                                                                                                                               |
...
| **4b**     | ✅ shipped         | `cd974b4` | `@cir/components` (13 baseline) + `@cir/react` (provider, hooks, walker, confirm portal). Next.js App Router compatible (`'use client'` everywhere needed)                                                                                           |
...
| **5a**     | 🟡 **in progress** | —         | `@cir/compiler` real source: Gemini integration, prompt builder, server-side ManifestStore (Tier 3), `BrandKit` in `@cir/schemas`, `respects_brand_kit` policy, `<DebugPanel>` + `<CompileBadge>` UI, `/api/cir/*` route handlers, fallback compiler |
| **5b**     | 📋 planned         | —         | `apps/demo-dummyjson` — 8 capabilities composing, lens-switching showcase. Real LLM compiles per intent                                                                                                                                              |
| **5c**     | 📋 planned         | —         | `apps/demo-github` skeleton — 11 capabilities, optional `GITHUB_TOKEN`, rate-limit policy, read-only routes                                                                                                                                          |
| **5d**     | 📋 planned         | —         | GitHub demo mutations: `verbal_required` confirmation (extend ConfirmPortal), real PR/issue actions, audit log dashboard                                                                                                                             |
| **5e**     | 📋 planned         | —         | Cross-cutting polish: Playwright for both new demos, deployment notes, root README finalized                                                                                                                                                         |

**Cumulative numbers (post-4d):** 7 packages · 66 test files · 385 tests · 100% coverage on schemas/policies, 90%+ on runtime/components/react · `pnpm validate` exits 0 in ~6s.
...
│   └── compiler/               #   @cir/compiler (LLM compile service — Phase 5a)
├── apps/
│   ├── demo/                   # email-triage offline showcase (shipped)
│   ├── demo-dummyjson/         # lens-switching showcase (Phase 5b)
│   └── demo-github/            # real-mutations showcase (Phase 5c-d)
