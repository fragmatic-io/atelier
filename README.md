...
7. Publish a reviewed surface, choose its page/placement, and generate the one-script hosted embed. Atelier serves the UI/runtime; reviewed API calls run directly in the customer's browser through its existing same-origin session.
8. Verify the mount, hosted UI, browser API allowlist, customer API authorization, tool behavior, confirmation policy, and browser states before production.
...
| `platform/packages/surface-install`, `embed` | One-script hosted UI installation, reviewed browser API calls, and factual receipts |
