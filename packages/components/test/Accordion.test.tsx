...
describe('Accordion', () => {
  ...
  it('defaults to variant=bordered', () => {
    const { container } = render(<Accordion items={ITEMS} />);
    expect(
      container.querySelector('[data-cir-component="Accordion"]')?.getAttribute('data-variant'),
    ).toBe('bordered');
  });
  it('reflects each variant on data-variant', () => {
    for (const v of ['bordered', 'elevated', 'ghost', 'tinted'] as const) {
      const { container, unmount } = render(<Accordion items={ITEMS} variant={v} />);
      expect(
        container.querySelector('[data-cir-component="Accordion"]')?.getAttribute('data-variant'),
      ).toBe(v);
      unmount();
  ...
  it('applies the elevated variant class', () => {
    const { container } = render(<Accordion items={ITEMS} variant="elevated" />);
    const root = container.querySelector('[data-cir-component="Accordion"]') as HTMLElement;
