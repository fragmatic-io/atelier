...
- `check-license-headers.ts` — verifies every in-scope source file starts with the SPDX Apache-2.0 header. Wired into `pnpm validate` via the `check:license-headers` script. Run with `--fix` (`pnpm fix:license-headers`) to prepend missing headers.
