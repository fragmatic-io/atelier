...
- `check-license-headers.ts` — verifies every in-scope source file starts with the SPDX MIT header (legacy `Apache-2.0` headers are still accepted but are a hold-over from the original relicense; new files should use MIT). Wired into `pnpm validate` via the `check:license-headers` script. Run with `--fix` (`pnpm fix:license-headers`) to prepend missing headers.
