...
`schemas/` is populated — 13 generated JSON Schemas covering every CIR artifact type, regenerated by `pnpm schemas:dump`. `cir.json` ships with this repo and advertises the small reference set of capabilities, skills, policies, and recipes used by the demo. Re-author `cir.json` whenever an artifact lands, is removed, or is version-bumped.
