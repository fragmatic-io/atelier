...
- `schemas/` — generated JSON Schemas for Capability, Skill, Component, Manifest, Trigger, Intent, Audit, etc. Generated from `@atelier/schemas` via `pnpm schemas:dump`. Re-run after any schema change. The golden test in `packages/schemas/test/golden.test.ts` guards against drift.
...
`schemas/` is populated — 13 generated JSON Schemas covering every Atelier artifact type, regenerated by `pnpm schemas:dump`. `cir.json` ships with this repo and advertises the small reference set of capabilities, skills, policies, and recipes used by the demo. Re-author `cir.json` whenever an artifact lands, is removed, or is version-bumped.
