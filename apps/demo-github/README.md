...
Real-mutations issue triage showcase. CIR-against-GitHub: optimistic archive,
...
- **Mono**: IBM Plex Mono / GitHub Mono (system fallback) — code refs, shas,
  ...
  inline reference (`#CIR-123`, `a1b2c3d`, etc.).
...
The kit lives in `lib/brand-kit.ts` (typed against `@cir/schemas`'s
`BrandKitSchema`) and is folded into the compiler service prompt via
`lib/cir-server.ts`. The `respects_brand_kit` policy enforces the same
...
```sh
pnpm --filter @cir/demo-github dev         # next dev
pnpm --filter @cir/demo-github typecheck   # tsc --noEmit
pnpm --filter @cir/demo-github lint        # no-op (root eslint covers apps)
pnpm --filter @cir/demo-github test        # vitest run
```
