...
**Default-on in the demo:** `apps/demo` wires `SubstringCapabilityResolver` (no API key required) and threads it onto every compile. Opt out via `ATELIER_CAPABILITY_RESOLVER=off`; legacy `CIR_CAPABILITY_RESOLVER_ENABLED=0` honoured for one release cycle:
