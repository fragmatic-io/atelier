...
The agent gets a system prompt, the recipe, the user's intent slice, and access to ten tools. It calls tools to discover what it needs, drafts a manifest, calls `validateDraft` against the policy engine, patches if violations come back, and emits when validate is ok. In production mode, the final manifest must match a draft that passed validation.
...
const llmCompiler = new ValidationFeedbackCompiler(
  new ToolUsingCompiler({ inner, env: { ...toolEnv, validate } }),
  { maxRetries: 2 },
);
...
```ts
import { ToolUsingCompiler, createBaselineManifestValidator } from '@atelier/compiler';

const components = loadComponents('./components/registry.json');
...
- ✅ **C-2** — Tool-using compiler. Ten tools, default-on in the demos; opt out via `ATELIER_COMPILER_TOOLS=off`. Legacy `CIR_COMPILER_TOOLS_ENABLED=0` honoured for one release cycle.
- ✅ **C-3 / S-1** — Capability scoping. Two-stage compile: small resolver picks a bounded set from summaries; the full compiler gets those schemas.
...
- [Compiler → Tools](/compiler/tools/) — the ten tools the agent calls.
